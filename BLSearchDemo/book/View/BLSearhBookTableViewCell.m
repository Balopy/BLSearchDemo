...
- (void) setupSubViews {
    ...
    self.bookImgView = [BLGeneralControl createUIImageViewWithName:@"默认加载图" toView:self.contentView];
    self.bookImgView.backgroundColor = [UIColor redColor];
    ...
    self.lineView = [BLGeneralControl createUIViewWithBackColor:[UIColor lightGrayColor] fromVeiew:self.contentView];
}
...
- (void)setModel:(BLSearchBookListModel *)model {
    _model = model;
    UIImage *image = MMIMAGENAMED(@"默认加载图");
    image = [image circleImageWithSize:CGSizeMake(40, 40) radius:20 borderWidth:5 borderColor:[UIColor whiteColor]];

    [self.bookImgView setImage:image];
    ...
    NSString *number = [NSString stringWithFormat:@"%d 节课", arc4random()%1000];
    self.courseNumLab.text = number;
    
    
    NSString *alreadyBuy = [NSString stringWithFormat:@"%d 人已购买", arc4random()%1000];
